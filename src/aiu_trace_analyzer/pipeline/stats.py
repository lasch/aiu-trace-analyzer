...
class StatsExtractionContext(EventPairDetectionContext, PipelineContextTool):
    ...
    def __del__(self) -> None:
        for pid, data in self.total_util.items():
            ...
            aiulog.log(aiulog.INFO, f'UTL: pid={pid}: avg_mm_util={round(mean_pt_util, 3)}, avg_util={round(total_pt_util,3)}, t_mm={tot_matmul}, t_sf={tot_other}, ratio_mm/sf={round(tot_matmul/total*100.0, 2)}:{round(tot_other/total*100.0, 2)}')
    ...
    def drain(self) -> list[TraceEvent]:
        ...
        for pid in sorted(stats_list) :
            ...
            for stats,name,stats_ts1ts2,stats_ts2ts3,stats_ts3ts4,stats_ts4ts5 in sorted(stats_list[pid],key=lambda x: x[0][1],reverse=True) :
                ...
                if fp_stats != 0 :
                    ...
                    fp_stats.write("%5.2f\t  %9.3f\t %5d\t %8.3f\t %8.3f\t %8.3f\t %8.3f\t %8.3f\t %4d\t %s\n" %(stats[0], stats[1], stats[2], stats[3], stats[4], stats[5], stats[6], stats[7], pid, name))
            ...
            if fp_active != 0 :
                #     Total Kernel time Elapsed Time  Start Time   End Time    Active percentage   pid
                fp_active.write("%9.3f\t  %9.3f\t  %9.3f\t %9.3f\t %5.2f\t %4d\n" %(total_compute_time, elapsed_time, self.min_ts_map[pid], self.max_ts_map[pid], active_percentage, pid))
