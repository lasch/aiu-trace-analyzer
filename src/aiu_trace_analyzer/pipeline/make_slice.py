...
class SliceCreationContext(EventPairDetectionContext):
    ...
    def slice(self, event: TraceEvent) -> list[TraceEvent]:
        ...
        if parter:
            ...
            aiulog.log(aiulog.DEBUG, "SLICE POP:", idx, event["name"], event["ts"], "remlen=", len(self.queues[queue_id]))
            return self.emit_as_complete([ parter, event ])
        aiulog.log(aiulog.WARN, "SLICE END: No opening event found.", idx, event["name"], event["ts"], "remlen=", len(self.queues[queue_id]))
        return []
