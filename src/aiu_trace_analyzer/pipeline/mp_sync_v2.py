...
class MpTsCalibV2Context(EventPairDetectionContext):
    ...
    def mp_calibrate_dts( self ) -> None:
        ...
        TODO: use numpy
        '''
        dts_ranges_per_pid_cg = {}      # <min, max> dev-ts ranges for each coll-group on each rank/proc,
                                        #       <min> is not in effect in mp_sync_v2
        ref_hts_per_pid_cg    = {}      # ref_hts, reference host-ts for host-ts dev-ts calibration
    ...
    #
    #   Derivation: ( t_p1_syncd_to_p0 - t5_ref_p0 ) / ( t_p1 - t5_ref_p1 ) = D_p1 = (t5_cgN_p1 - t5_cg0_p1) / (t5_cgN_p0 - t5_cg0_p0)
