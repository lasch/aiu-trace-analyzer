...
class MpCalcBwContext(AbstractHashQueueContext):
    ...
    def calc_bw( self ):
        ...
        for i, (_ts_end, e_idx) in enumerate( self.all_events_w_ts_end ):
            ...
            if e["ph"] in ["X", "b"] and "args" in e and "CollGroup" in e["args"] and "AllReduce_all_reduce_" in e["args"]["CollGroup"]:
                if not in_coll_gr:
            ...
            # (2) generate counter events for "BW allreduce" and "BW all-pcie"
                ...
                in_coll_gr = False; got_bytes = False; num_bytes = 0
                recv_count = 0

            elif in_coll_gr and got_bytes and "args" in e and "CollGroup" in e["args"] and "AllGather" in e["args"]["CollGroup"]:
                break
