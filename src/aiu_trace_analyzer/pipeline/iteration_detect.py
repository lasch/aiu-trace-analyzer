...
class IterationDectectContext(AbstractHashQueueContext):
    ...
    def detect_iteration(self, event:TraceEvent):
        ...
        if i_stat.previous_ts == 0.0:
            ...
            detected = len(new_ref_word) < len(i_stat.referernce_word)  # if the returned word is shorter, we know we found a repeating sequence
            ...
            if detected:
                ...
                i_stat.previous_ts = i_stat.collected_ts[pref_len+rword_len]             # prev_ts to be set to where the current collected word starts
                i_stat.avg_time = i_stat.previous_ts - i_stat.collected_ts[pref_len]     # first entry: mean == value
            ...
            if len(i_stat.collected_word) > len(i_stat.referernce_word):
                aiulog.log(aiulog.DEBUG, "ITSTATS: NO_MATCH for iteration. Kernel sequences interupted by missing/additional event. Will retry with later iteration.")
    ...
    def detect_reference_word(self, word:str) -> tuple[int, str]:
        ...
        Challenge: the could be prefix/one-off kernels running before the first iteration that need to be detected and filtered out
        '''
        wlen = len(word)
        if wlen < 2*self.min_run_len:
            return 0,word

        max_prefix = int(math.floor(wlen / 3))  # if ref and cmp need to have equal len, then a prefix can have a max len of 1/3 of the total len
    ...
    def try_skip_kernel(self, i_stat: IterationStatus) -> IterationStatus:
        ...
        while rem_len > 0:
            if i_stat.referernce_word[:rem_len] != i_stat.collected_word[skip_letters:skip_letters+rem_len]:
                ...
                aiulog.log(aiulog.DEBUG, "ITSTATS:", f'detected new iteration by skipping {skip_letters}, match_len {rem_len}')
                aiulog.log(aiulog.TRACE, "ITSTATS: COLLECTED:", i_stat.collected_word)
                return i_stat
    ...
    def drain(self) -> list[TraceEvent]:
        for pid, data in self.queues.items():
            aiulog.log(aiulog.INFO, "ITSTATS:", f'[{pid}] avg_iteration_time={data.avg_time}, iterations={data.iterations}')
        return []
