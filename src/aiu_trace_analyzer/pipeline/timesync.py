...
def _assign_ts_dur(a:int, b:int, ts_list:list[float], overlap_tolerance=1.0/560.0) -> tuple[float, float]:
    '''
    NOTE: takes the TS[1-5] numbered timestamps a,b and returns/computes the wallclock ts and dur from the ts_list using adjusted indices
...
def _match_opIds_from_event( event: TraceEvent ):
    ...
    op_ids      = np.nonzero( np_op_ids == True )[0]
    return op_ids
