...
class PTorchToFlexTimeMapping(object):
    ...
    def compute_offset(self) -> None:
        ...
        if len(self.flex_tsdur) == 0 or len(self.ptorch_tsdur) == 0:
            aiulog.log(aiulog.DEBUG, "TALN", 'Flex Time Offset is 0.0 because either Flex or Torch data was not detected.')
            self.flex_offset = 0.0
            return

        if len(self.flex_tsdur) != len(self.ptorch_tsdur):
            aiulog.log(aiulog.WARN, "TALN", 'Matching Flex and Torch events will be based on different number of entries. This could result in mis-alignment. Make sure the correct traces are ingested.')
        ...
        # make sure the torch duration is at least as long as the flex duration, otherwise, the 2 events don't fit together
...
class TimeAlignmentContext(AbstractHashQueueContext):
    ...
    def collect_rank_pairs(self, event:TraceEvent) -> None:
        ...
        if self._FLEX_REFERENCE_EVENT_NAME in event["name"]:
            aiulog.log(aiulog.TRACE, "TALN", f'flex reference event: {queue_id} {event["ts"]}, {event["dur"]}, {event["name"]}')
        ...
        for prio,candidate in enumerate(self._TORCH_REFERENCE_EVENT_NAME_LIST):
            if candidate in event["name"]:
                aiulog.log(aiulog.TRACE, "TALN", f'ptorch reference event: {queue_id} {event["ts"]}, {event["dur"]}, {event["name"]}')
    ...
    def check_and_merge_single_rank(self):
        ...
        if len(self.is_multi_rank) == 2:
            ...
            aiulog.log(aiulog.INFO, "TALN", 'found 2 pids that appear belong to a single rank. Assuming single-process run and spreading data to both pids.')
