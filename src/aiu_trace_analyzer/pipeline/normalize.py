...
class NormalizationContext(AbstractHashQueueContext):
    ...
    def __del__(self) -> None:
        ...
        if math.isclose(mean, 0.0, abs_tol=1e-9):
            return
        if ma-mi > mean * 0.2:
            aiulog.log(aiulog.WARN, f"FREQ: Min/Max of detected correct frequency is >20% of mean ({round(mi,3)},{round(ma,3)}). This indicates some events might have been assigned to the wrong TSx epoch.")
        elif abs(mean - self.soc_frequency) > 0.1:
            aiulog.log(aiulog.WARN, f"FREQ: Recommendation: to minimize event time drift (max: {madr}us) between CPU and Accelerator, use:", f"--freq={round(mean,3)}")
    ...
    def get_overflow_count(self, qid, job:str, ts: float, cycle: int) -> tuple[float, float, float]:
        ...
        if job not in self.queues[qid]:
            ...
            job_drift = int(epoch_start - (self.queues[qid]["0"][0] + elapsed_epochs * self.OVERFLOW_TIME_SPAN_US))
            actual_freq = (abs_cycle - self.queues[qid]["0"][2]) / ( ts - self.queues[qid]["0"][1]) if ts != self.queues[qid]["0"][1] else None
    ...
    def tsx_32bit_local_correction(self, qid, event: TraceEvent) -> dict:
        if "TS1" in event["args"]:
            ...
            if event["dur"] > self.OVERFLOW_TIME_SPAN_US:
                aiulog.log(aiulog.WARN, "OVC: Detected event with long duration and thus potential undetected overflow in TSx counter.")
            return args
        return event["args"]
    ...
    def tsx_32bit_global_correction(self, qid, event: TraceEvent) -> dict:
        if "TS1" in event["args"]:
            args = event["args"]
            ovc, drift, tofix = self.get_overflow_count(qid, str(event["args"]["jobhash"]), event["ts"], int(event["args"]["TS1"]))
